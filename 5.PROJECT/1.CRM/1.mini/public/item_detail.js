const itemId = window.location.pathname.split('/').pop(); // 주소를 가져와서 / 로 짤라서 맨 뒤에꺼 반환
...
function renderTable(data) {
    ...
    headers.forEach(h => {
        ...
        if (h !== 'Month' && h !== 'TotalRevenue' && h !== 'ItemCount') {
    ...
    for (const [key, value] of Object.entries(data[0])) {
        ...
        if (key !== 'Month' && key !== 'TotalRevenue' && key !== 'ItemCount') {
...
function renderMonthRevenue(data) {
    ...
    const tableHeader = document.getElementById('table-h-item');
    const tableBody = document.getElementById('table-b-item');

    tableHeader.innerHTML = '';
    tableBody.innerHTML = '';

    console.log('renderMonthRevenue');
